    ...
    <div className="p-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between sticky top-0 bg-white/80 dark:bg-gray-900/80 backdrop-blur z-10">
      ...
      <div className="flex gap-2 items-center">
        ...
        <select
          ...
          disabled={summarizing}
          className="appearance-none px-2 py-1.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-[10px] font-medium text-gray-600 dark:text-gray-300 outline-none focus:ring-2 focus:ring-purple-500/20 cursor-pointer"
        >
        ...
        {hasOcrData ? (
          <button onClick={onIdentifySource} className="p-2 hover:bg-orange-50 dark:hover:bg-orange-900/20 rounded-lg text-gray-400 hover:text-orange-500">
            <BookOpen className="w-5 h-5" />
          </button>
        ) : sourceUrl ? (
          <button onClick={() => window.open(sourceUrl, "_blank")} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg text-gray-400">
            <ExternalLink className="w-5 h-5" />
          </button>
        ) : null}

        <button onClick={onCopyMarkdown} className="p-2 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded-lg text-gray-400 hover:text-emerald-500" title="Copy Markdown">
        ...
        <button 
          onClick={() => window.open(`https://research-mate-website.vercel.app/dashboard?id=${itemId}`, "_blank")} 
          className="p-2 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg text-gray-400 hover:text-blue-500"
          title="Open in Web Dashboard"
        >
          <LayoutTemplate className="w-5 h-5" />
        </button>

        <button onClick={onDelete} className="p-2 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg text-gray-400 hover:text-red-500">
